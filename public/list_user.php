...
<body>
	...
	<p>
		<div class="d-flex align-items-center justify-content-center pt-5">
			...
			<form action="http://localhost/admin/users" method="get" align="center">
		...
		<?php
			...
			if(isset($_GET['email']))
			{
				...
				if($userEmailInput)
				{
					...
					{
						$results = $database->searchUserByEmail($databaseConnection, $userEmailInput);
						if(is_int($results))
						{
							if($results == -1)
							{
			...
			else
			{
				...
				$rowCount = $database->countUsers($databaseConnection);
				if(is_int($rowCount))
				{
					if($rowCount == -1)
					{
				...
				$results = $database->getUserDetails($databaseConnection, $offset, $limit);
				if(is_int($results))
				{
					if($results == -1)
					{
